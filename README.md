...
>
> 沐曦MetaX GPU国产算力 · Lingshu-32B大模型 · VibraImage前庭振动 · 多智能体AIGC协作 · 心理学学科交叉教学
>
...
| **指导老师** | 刘寿强 |
| **参赛单位** | 华南师范大学 |
| **比赛名称** | 第八届全球校园人工智能算法精英大赛 · 算法创新赛 · 赛题5「AI+学科交叉」 |
...
- **经典量表数据参照**：内置 SAS/SDS/SCL-90/PSS-10/PANAS 标准化量表（自动计分 + CAT 自适应），作为第三模态效度基准，弥补纯 AI 输出概率性带来的效度漂移风险
- 加权融合策略（面部0.6 + 前庭0.4），置信度差异>35%自动复核；可用标注数据通过 `scripts/calibration/run_calibration_demo.py` 校正权重
...
### 📋 心理量表自评（多模态第三维度）
- **五种标准化量表**：SAS（焦虑自评，20题）、SDS（抑郁自评，20题）、SCL-90（症状自评，90题/10维度）、PSS-10（知觉压力，10题）、PANAS（正负性情绪，20题）
- **自动计分引擎**：支持反向题、标准分公式（×1.25 / 总均分×100 / 维度标准分）、0-base 计分（PSS-10）、SCL-90 十维因子分与 PANAS 双维度得分
- **CAT 自适应测验（教学演示版）**：基于 IRT 3PL 模型的「最大信息量」逐题推送，通常仅需全卷约 1/3 题量即可出结果（`/api/scales/cat/start`、`/api/scales/cat/next`）
- **AI交叉验证 + 统计检验**：`GET /api/scales/crosscheck/{student_id}` 对比量表结果与 AI 情绪数据；`GET /api/scales/validation/summary` 输出全库 Pearson r / Cohen's Kappa / 灵敏度·特异度
- **种子数据**：每个学生预种 2 份量表数据（SAS/SDS，按标准题库计分），仪表盘和 AIGC 报告自动引用
- 量表题目来源于公开发表学术论文（Zung, 1965/1971; Derogatis, 1975; Cohen, 1983; Watson, 1988），遵循学术使用规范

### 🧪 心理教学实验组件（学科交叉 ★）
- **E1–E12 参数解释卡**：每个前庭振动参数对应心理学构念（攻击性/压力/紧张/抑郁/幸福等），可直接对照心理测量课程知识点
- **个体 vs 常模雷达图**：以 10,266 人常模为基线可视化个体 Z 分
- **K 值情绪状态指数**：|K|<3 稳定 / 3–6 关注 / ≥6 预警
- **实验指导书**：内置"实验目的→设备→程序→记录表→结果解释"教学流程
- **实验报告生成器**：`POST /api/aigc/report/experiment` 一键生成符合心理测量课程规范的《情绪测量实验报告》（实验目的/方法/结果/讨论/结论）
- 前端页面：`/experiment`（教学实验）

### 📈 试点工具链（应用效果证据）
- `GET /api/admin/pilot/export` — 按班级/年级分组导出试点数据 CSV
- `GET /api/admin/pilot/compare` — 系统 vs 人工计分耗时对比等应用效果指标
- `GET /api/admin/pilot/report` — 聚合试点数据+反馈的成果报告摘要
- `GET /api/vibraimage/norms` — 常模数据端点（教学可视化用）
...
| Prompt 解析器 | 12 | 100% | LLM 输出解析、工具白名单、容错 JSON |
| 量表测评（含 CAT） | 21 | 100% | 五套量表计分、SCL-90 维度、PSS-10 0-base、CAT 引擎、统计检验、实验报告 |
| VibraImage 引擎（含端到端流水线） | 17 | 100% | 帧差分/频率/直方图/情绪参数/合成视频全流水线 |
| 升级模块（三模态证据融合/分级响应模型自适应测验/合成数据/信效度） | 21 | 100% | 合成被试、证据融合、多项似然、内部一致性系数、消融对照 |
| 虚拟被试教学闭环 | 16 | 100% | 剖面库、生成器（隐藏真值）、自动批改（满分/半分/零分）、API 端点 |
| 情绪预测与异常检测 | 14 | 100% | 线性趋势预测、加权移动平均、Z-score点异常、CUSUM漂移、API端点 |
| **合计** | **127** | **100%** | **全模块覆盖** |
...
# 预期输出
# tests/test_scales.py::TestScoring::test_sas_known_answer PASSED
# ...
# ==================== 127 passed in 5.6s ====================
```
...
> **比赛信息**: 第八届全球校园人工智能算法精英大赛 · 算法创新赛 · 赛题5「AI+学科交叉」
>
> **技术底座**: 基于国产算力平台（沐曦MetaX GPU / Gitee.AI）的 AIGC 智能体 + 心理测量学科交叉平台
>
> **算力支持**: 曦云C500 (MetaX) 本地GPU加速 + moark.com (MetaX) Lingshu-32B 云端推理
>
> **版本**: v2.4.0 — AI+学科交叉教学组件（五套心理量表+CAT自适应测验、教学实验可视化、试点工具链）
>
