...
class FaceDetector:
    ...
    def _load_haar_cascade(self):
        ...
        新版OpenCV（>=5.0）可能不再捆绑 haarcascade XML，此时退化为
        「画面中央区域 ROI」模式（假定被试坐于摄像头中央），保证引擎不崩溃。
        """
        import os
        candidates = []
        if cv2.data is not None:
            candidates.append(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        # 项目内备份路径（可手动放置 xml 启用真实 Haar 检测）
        candidates.append(
        ...
        if cascade_path is None:
            ...
            self._use_haar = True
            logger.warning(
                "Haar Cascade XML 不可用（当前OpenCV未捆绑），退化为画面中央ROI模式。"
                "可安装 ultralytics(YOLO) 或放置 %s 启用真实人脸检测。",
                candidates[1],
            )
            return
        self._face_cascade = cv2.CascadeClassifier(cascade_path)
        self._use_haar = True
        logger.info("使用Haar Cascade人脸检测: %s", cascade_path)
    ...
    def _detect_haar(self, frame: np.ndarray) -> Optional[Tuple[int, int, int, int]]:
        ...
        if self._face_cascade is None:
            # 退化模式：假定被试位于画面中央，取中央 60% 宽 × 45% 高 区域
            H, W = frame.shape[:2]
            x1 = int(W * 0.20)
            x2 = int(W * 0.80)
            y1 = int(H * 0.05)
